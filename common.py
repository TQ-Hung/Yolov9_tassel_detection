...
class SimAM(nn.Module):
    def __init__(self, lambda_coeff=0.5):  # Thêm tham số lambda_coeff với giá trị mặc định
        super(SimAM, self).__init__()
        self.lambda_coeff = lambda_coeff  # Lưu giá trị lambda_coeff

    def forward(self, X):
        n = X.shape[2] * X.shape[3] - 1
        d = (X - X.mean(dim=[2, 3])).pow(2)
        v = d.sum(dim=[2, 3]) / n
        E_inv = d / (4 * (v + self.lambda_coeff)) + 0.5
